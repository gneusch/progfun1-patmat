...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      @tailrec
      def createList(chars: List[Char], finalList: List[(Char, Int)]): List[(Char, Int)] = {
        if(chars.isEmpty){
          finalList
        }
        else {
          val char = chars.head
          @tailrec
          def finalListCreator(innerListHelper: List[(Char, Int)], innerFinList: List[(Char, Int)], inserted: Boolean = false): List[(Char, Int)] ={
            if (innerListHelper.isEmpty) {
              if(inserted){
                innerFinList
              }
              else {
            ...
            else if( innerListHelper.head._1 == char ) {
              finalListCreator(innerListHelper.tail, (innerListHelper.head._1, innerListHelper.head._2 + 1) :: innerFinList, true)
            }
            else{
              finalListCreator(innerListHelper.tail, innerListHelper.head ::innerFinList, inserted)
            }
          }
          val finalListHelper = finalListCreator(finalList, List[(Char, Int)]())
          createList(chars.tail, finalListHelper)
        }
      }
      createList(chars, List[(Char, Int)]())
    }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      @tailrec
      def toLeafList(freqs: List[(Char, Int)], leafList: List[Leaf]): List[Leaf] = freqs match {
        case x :: Nil => new Leaf(x._1, x._2) :: leafList
        case x :: xs => toLeafList(xs, new Leaf(x._1, x._2) :: leafList)
      }
      def insert(leaf: Leaf, leafList: List[Leaf]): List[Leaf] = leafList match{
        case List() => List(leaf)
        case x :: xs => if(leaf.weight <= x.weight) leaf :: leafList else x :: insert(leaf, xs)
      }
      def sort(leafList: List[Leaf]): List[Leaf] = leafList match {
        case List() => List()
        case x :: xs => insert(x, sort(xs))
      }
      sort(toLeafList(freqs, List()))
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case x1 :: Nil => trees
      case x1 :: x2 :: xs => new Fork(x1, x2, chars(x1) ::: chars(x2), weight(x1) + weight(x2)) :: xs
    }
   ...
   */
    def createCodeTree(chars: List[Char]): CodeTree = {
      ...
      codeTreeList match {
        case x :: Nil => x
        case _ => throw new Error("codeTreeList contains more than one element! " + codeTreeList)
      }
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def traverseTree(tree: CodeTree, charToFind: Char, bits: List[Bit]): List[Bit] = tree match {
      ...
      case Fork(left, right, _, _) => {
       ...
       else if(chars(left).contains(charToFind)) traverseTree(left, charToFind, bits :+ 0)
       else throw new Error("No char: " + charToFind + " found in the codetree.")
      }
    }
    def textIter(text: List[Char], bits: List[Bit]): List[Bit] = {
      if(text.isEmpty) bits
      else {
        textIter(text.tail, bits ::: traverseTree(tree, text.head, List()))
      }
    }
    textIter(text, List())
  }
